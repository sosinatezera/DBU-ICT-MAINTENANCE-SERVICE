      ...
      verifySmtp()
        .then((r) => {
          if (r.ok) console.log(`  Email : ${r.detail}`);
            ...
            console.log(
              "  Email : SMTP configured but connection failed — messages will be skipped (see scripts/verify-smtp.js).",
            );
        })
        .catch(() => console.log("  Email : SMTP check skipped (non-fatal)."));
    } else {
      console.log(
        "  Email : SMTP not configured — email delivery disabled (optional, see backend/.env).",
      );
    }
  } catch (err) {
